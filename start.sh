...
if [ -e "$wtr_path" ]; then
    ...
    if [ "$time_difference" -gt 43200 ]; then
        ...
        # Remove the &>/dev/null if you want it to log on the splash screen (for diagnostics, I guess)
        echo "File is already new." &>/dev/null
    fi
else
    # File not found, so downloading the file.
    # Remove the &>/dev/null if you want it to log on the splash screen (for diagnostics)
